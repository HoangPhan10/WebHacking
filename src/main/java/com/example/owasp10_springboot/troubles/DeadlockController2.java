...
public class DeadlockController2 extends AbstractController {
    ...
    private List<UserBug> selectUsers(String order, Locale locale, ModelAndView mav) {
        List<UserBug> users = null;
        try {
            users = jdbcTemplate.query("select * from users where ispublic = 'true' order by id "
                    + ("desc".equals(order) ? "desc" : "asc"), new RowMapper<UserBug>() {
    ...
    private void updateUsers(List<UserBug> users, Locale locale, ModelAndView mav) {
